...
// std::shared_ptr<Matrix> finite_diff(std::shared_ptr<Layer> L){
//    double delta = 1e-7;
//    std::shared_ptr<Matrix> output;
//    std::shared_ptr<Layer> L2(L->getWeights());
//    output = std::make_shared<Matrix>(L2->forward()->weights);
//    return output;
...
int main(int argc, char *argv[])
{
    ...
    std::vector<std::vector<double>> weights1 = readCSV<double>("../data/wights_data_Layer1_3n.csv");
    std::vector<std::vector<double>> weights2 = readCSV<double>("../data/wights_data_Layer2_3n.csv");
    ...
    // std::cout << "dimension of y_transposed(targets) : (" << y_transpose->getNumRows() << "," << y_transpose->getNumCols() << ")" << std::endl;
    y->print();
    ...
    // First Layer
    std::shared_ptr<Layer> L1 = std::make_shared<Layer>(2, 3, weights1);

    // Second Layer
    std::shared_ptr<Layer> L2 = std::make_shared<Layer>(3, 3, weights2);
    ...
    if (mode == "train")
    {
        ...
        for (int epoch = 0; epoch < epochs; epoch++)
        {
            ...
            //std::cout << "dimension of output_softmax(predictions) : (" << output_softmax->getNumRows() << "," << output_softmax->getNumCols() << ")" << std::endl;

            // Loss
            double loss = loss_fct.crossEntropyLoss_forward(output_softmax, y);//Changed y_transposed to y so that y has the same number of rows as output_softmax
