...
std::shared_ptr<Matrix> Matrix::operator+(const Matrix &mat2) const
{
    auto result = std::make_shared<Matrix>(numRows, numCols);
    for (int i = 0; i < numRows; ++i)
    {
        for (int j = 0; j < numCols; ++j)
        {
            result->values[i][j] = values[i][j] + mat2.values[i][j];
        }
    }
    return result;
}
...
std::shared_ptr<Matrix> Matrix::dotProduct(const Matrix &mat) const
{
    ...
    auto result = std::make_shared<Matrix>(numRows, mat.numCols);
    for (int i = 0; i < numRows; ++i)
    {
        for (int k = 0; k < numCols; ++k)
        {
            ...
            {
                result->values[i][j] += temp * mat.values[k][j];
            }
        }
    }
    return result;
